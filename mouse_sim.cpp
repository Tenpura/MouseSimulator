...
int g_need_queue_size = 0;		//リングキューの最大値
uint32_t g_sum_queue_size_node = 0;		//ループ回数
int g_sum_queue_size_cell = 0;
int g_mouse_x, g_mouse_y;


static const int console_area = 400;  //スクロールより右部分にコンソール用の空間をどれくらい用意するか
int window_width = 1500;	//画面の解像度

int window_length = 800;	//画面の解像度
...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance,LPSTR lpCmdLine, int nCmdShow )
{
	...
	while(ProcessMessage()==0 && ScreenFlip()==0 && ClearDrawScreen()==0 && GetHitKeyStateAll(key)==0){	
		...
		DrawFormatString(maze_area_width + 10, 50, white, "need queue size (node) -> %d", g_need_queue_size);
		DrawFormatString(maze_area_width + 10, 70, white, "max loop count (node) -> %d", g_sum_queue_size_node);
		DrawFormatString(maze_area_width + 10, 90, white, "max loop count (cell) -> %d", g_sum_queue_size_cell);
		DrawFormatString(maze_area_width + 10, 110, white, "now min step -> %d", now_node.get_step(g_mouse_x,g_mouse_y,now_node.get_min_compas(g_mouse_x,g_mouse_y)));
